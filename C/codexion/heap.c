...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/07/04 23:43:38 by bgomes-b          #+#    #+#             */
/*   Updated: 2026/07/04 23:55:29 by bgomes-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	heap_push(t_heap *heap, t_request req, t_scheduler sched)
{
	...
	{
		parent = (i - 1) / 2;
		if (!comp_higher_priority(heap->requests[i],
				heap->requests[parent], sched))
			break ;
...
void	heap_sift_down(t_heap *heap, int i, t_scheduler sched)
{
	...
	while (1)
	{
		...
		top = i;
		if (left < heap->size
			&& comp_higher_priority(heap->requests[left],
				heap->requests[top], sched))
			top = left;
		if (right < heap->size
			&& comp_higher_priority(heap->requests[right],
				heap->requests[top], sched))
			top = right;
		if (top == i)
			break ;
