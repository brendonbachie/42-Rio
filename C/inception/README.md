...
Persistent data lives in two **named volumes** whose host storage is under `/home/bgomes-b/data`. Containers communicate on a custom Docker bridge network named `inception`. Secrets (DB passwords and WordPress credentials) are mounted via **Docker secrets**; non-sensitive configuration uses a `.env` file.
...
- `make`
- Host directories `/home/bgomes-b/data/wordpress` and `/home/bgomes-b/data/mariadb`
- DNS or `/etc/hosts` entry: `127.0.0bgomes-b.1 bgomes-b.42.fr`
...
Open **https://bgomes-b.42.fr** (accept the self-signed certificate). WordPress admin: **https://bgomes-b.42.fr/wp-admin**.
